...
parser.add_argument("--dataset_root", type=str, help='path to dataset folder containing train-test-validation folders', default='/vinai/phongtt15/models/FEDVR/results/vox5000_woTSA_woLM_woFI_train')
parser.add_argument("--checkpoint_dir", type=str, default='./checkpoint_phong', help='path to folder for saving checkpoints')
...
parser.add_argument("--validation_batch_size", type=int, default=8, help='batch size for validation. Default: 10.')
...
parser.add_argument("--progress_iter", type=int, default=5, help='frequency of reporting progress and validation. N: after every N iterations. Default: 100.')
...
if torch.cuda.device_count() > 1:
    ...
    flowComp = nn.DataParallel(model.UNet(6, 4), device_ids=[0, 1, 2])
    # flowComp = model.UNet(6, 4)
    flowComp.to(device)
    ArbTimeFlowIntrp = nn.DataParallel(model.UNet(20, 5), device_ids=[0, 1, 2])
    ArbTimeFlowIntrp.to(device)
    ###Initialze backward warpers for train and validation datasets
    trainFlowBackWarp      = nn.DataParallel(model.backWarp(352, 352, device), device_ids=[0, 1, 2])
    trainFlowBackWarp      = trainFlowBackWarp.to(device)
    validationFlowBackWarp = nn.DataParallel(model.backWarp(640, 352, device), device_ids=[0, 1, 2])
    validationFlowBackWarp = nn.DataParallel(model.backWarp(640, 352, device), device_ids=[0, 1, 2])
...
trainset = dataloader.SuperSloMo(root=args.dataset_root, transform=transform, train=True)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.train_batch_size, shuffle=True)

validationset = dataloader.SuperSloMo(root=args.dataset_root, transform=transform, randomCropSize=(640, 352), train=False)
...
### Main training loop
for epoch in tqdm(range(dict1['epoch'] + 1, args.epochs)):
    print("Epoch: ", epoch)
...
#         print('type', type(fCoeff), type(F_0_1))
        ...
        if ((trainIndex % args.progress_iter) == args.progress_iter - 1):
            torch.save(flowComp.state_dict(), '/vinai/thaontp79/phong_supp/Super-SloMo/trained_models/FC_{}.pth'.format(epoch))
            torch.save(ArbTimeFlowIntrp.state_dict(), '/vinai/thaontp79/phong_supp/Super-SloMo/trained_models/AT_{}.pth'.format(epoch))
#             torch.save(model.state_dict(), '/vinai/thaontp79/phong_supp/Super-SloMo/trained_models/{}.pth'.format(epoch))
            print('saved model')
            end = time.time()
...
#                 }
#         torch.save(dict1, '/vinai/thaontp79/phong_supp/Super-SloMo/ckpt/{}.ckpt'.format(checkpoint_counter))
